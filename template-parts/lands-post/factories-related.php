...
$transient_key = 'latest_land';
$lands_query = get_transient( $transient_key );

if ( false === $lands_query ) {
    $args = [
        'post_type'      => 'land',
        'posts_per_page' => 4,
        'orderby'        => 'date',
    ...
    $lands_query = new WP_Query( $args );
    set_transient( $transient_key, $lands_query, HOUR_IN_SECONDS );
}
?>
    ...
    <div class="container">
        ...
        <div class="swiper swiper3">
                        ...
                        <div class="swiper-wrapper">
                            <?php if ( $lands_query->have_posts() ) : ?>
                                <?php while ( $lands_query->have_posts() ) : $lands_query->the_post(); ?>



                                        <?php get_template_part('template-parts/single/lands-card'); ?>


                                <?php endwhile; ?>
                                <?php wp_reset_postdata(); ?>
                            <?php else : ?>
                                <p><?php esc_html_e( 'There are no lands currently available.', 'cob_theme' ); ?></p>
                                <p></p>
                            <?php endif; ?>
                        </div>
